...
export class SyncUtil {
  ...
  public getHashModFiles(): string {
    const modPath = this.modImporter.getModPath("Corter-ModSync");
    return this.vfs.readFile(path.join(modPath, "src", "hash.jsonc"));
  }

  public initHashModFiles() {
    const modPath = this.modImporter.getModPath("Corter-ModSync");
    const hash = JSON.stringify(
      this.hashModFiles(this.config.enabledSyncPaths)
    );
    const configPath = path.join(modPath, "src", "hash.jsonc");
    this.vfs.writeFile(configPath, hash, false);
    this.logger.info("Corter-ModSync: Hash file initialized");
  }
